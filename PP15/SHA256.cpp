...
void SHA256::transform() {
	...
	for (uint8_t i = 0, j = 0; i < 16; i++, j += 4) { // Разделить данные на 32-битные блоки для первых 16 слов
		m[i] = (m_data[j] << 24) | (m_data[j + 1] << 16) | (m_data[j + 2] << 8) | (m_data[j + 3]);
	}

	for (uint8_t k = 16; k < 64; k++) { // Осталось 48 блоков
...
void SHA256::revert(array<uint8_t, 32>& hash) {
	// SHA использует big-endian порядок байтов 
...
string SHA256::backToString(const array<uint8_t, 32>& digest) {
	stringstream s;
	s << setfill('0') << hex;
